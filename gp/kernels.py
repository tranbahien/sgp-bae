...
class MNISTKernel(Kern):

    def __init__(self, input_dim, variance=1.0, lengthscales=None,
                 active_dims=None, ARD=False, name=None, period=1.0,
                 K_obj_normalize=False, object_vectors=None):
        super(MNISTKernel, self).__init__(input_dim, active_dims, name=name)
        self.variance = parameter.PositiveParam(variance)
        if ARD:
            if lengthscales is None:
                lengthscales = torch.ones(input_dim)
        ...
        if object_vectors is not None:
            self.object_vectors = torch.nn.parameter.Parameter(
                object_vectors.data)
        else:
            self.object_vectors = object_vectors
    ...
    def Kdiag(self, X, X_inducing=False, presliced=False):
        ...
        return torch.diag(self.K(X, X, X_inducing, X_inducing))

    def K(self, X, X2=None, X_inducing=True, X2_inducing=True, presliced=False):
        if X2 is None:
            X2 = X
        ...
        X_view, X2_view = X[:, 1], X2[:, 1]
        if self.object_vectors is None:
            X_object, X2_object = X[:, 2:], X2[:, 2:]
        else:
            if X_inducing:
                X_object = X[:, 2:]
            else:
                X_object = self.object_vectors[X[:, 0].long()]
            if X2_inducing:
                X2_object = X2[:, 2:]
            else:
                X2_object = self.object_vectors[X2[:, 0].long()]

        X_view = X_view.unsqueeze(1)
        X2_view = X2_view.unsqueeze(1)
        
        f = X_view.unsqueeze(-2)
        f2 = X2_view.unsqueeze(-3)
        r = np.pi * torch.abs(f - f2) / self.period.get()
        scaled_sine = torch.sin(r) / self.lengthscales.get()
        view_matrix = self.variance.get() * torch.exp(-2 * scaled_sine**2)
        view_matrix = view_matrix.squeeze(-1)

        object_matrix = X_object @ X2_object.t()
        if self.K_obj_normalize:
            obj_norm = 1 / torch.matmul(
                torch.linalg.norm(X_object, dim=1, keepdim=True),
                torch.linalg.norm(X2_object, dim=1, keepdim=True).permute(1, 0))
